...
function Withdrawal() {
  ...
  // 3% processing fee — standard for grant disbursement admin costs
  const FEE_RATE = 0.03;
  ...
  const handleWithdrawSubmit = async () => {
    ...
    if (!withdrawAmount) { toast.error("Please enter a withdrawal amount"); return; }
    if (numAmount > profile.balance) { toast.error("Amount exceeds available balance"); return; }
    if (numAmount < 100) { toast.error("Minimum withdrawal is $100"); return; }
    setSubmitting(true);
    try {
            ...
            <div className="p-8 space-y-8">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="rounded-lg border border-gold/40 bg-gold/10 p-6">
                  ...
                  <p className="text-sm text-muted-foreground">Unlimited grant applications with full withdrawal access</p>
                </div>
              </div>
              <div className="rounded-lg border border-forest/20 bg-forest/5 p-6">
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-3">To upgrade to Tier 3 and enable withdrawals, you’ll need to:</p>
                    <ul className="space-y-2 text-sm text-muted-foreground">
                      <li className="flex items-start gap-2"><span className="text-forest font-bold">1</span><span>Link your bank account securely</span></li>
                      <li className="flex items-start gap-2"><span className="text-forest font-bold">2</span><span>Verify your identity with bank-level authentication</span></li>
                      <li className="flex items-start gap-2"><span className="text-forest font-bold">3</span><span>Wait for admin approval (usually 24 hours)</span></li>
                    </ul>
                  </div>
                </div>
              </div>
              <button onClick={() => navigate({ to: "/update-tier-3" })}
                ...
                <Lock className="h-4 w-4" /> Upgrade to Tier 3 <ChevronRight className="h-4 w-4" />
              </button>
        ...
        <main className="mx-auto max-w-3xl px-6 pt-10">
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
            <div className="bg-gradient-primary px-8 py-7 text-primary-foreground">
              ...
              <p className="mt-1 text-sm text-white/80">Withdraw your available balance. Funds will be sent after admin approval.</p>
            </div>
            <div className="p-8 space-y-8">
              ...
              {numAmount > 0 && (
                <div className="rounded-lg border border-border bg-accent/50 p-6 space-y-4">
                  ...
                  <div className="border-t border-border pt-4 flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">Admin Processing Fee (3%)</span>
              ...
              >
                {submitting ? <><Loader2 className="h-4 w-4 animate-spin" /> Processing…</> : <><Send className="h-4 w-4" /> Request Withdrawal</>}
              </button>

              <p className="text-center text-xs text-muted-foreground">
                A 3% admin processing fee covers wire transfer and compliance costs. No other charges apply.
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
            ...
            <div className="p-8 space-y-8">
              <div className="rounded-lg border border-forest/20 bg-forest/5 p-6 space-y-3">
                ...
                <div className="flex justify-between items-center py-3 border-b border-forest/20">
                  <span className="text-muted-foreground text-sm">Admin Processing Fee (3%)</span>
                  <span className="font-semibold text-destructive">
                    -${fee.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
              ...
              <div className="rounded-lg border border-gold/30 bg-gold/5 p-6">
                ...
                <ol className="space-y-3 text-sm text-muted-foreground">
                  {["Admin reviews your withdrawal request", "Approval typically takes 24–72 hours", "Funds transfer to your linked bank account", "You’ll receive email confirmation"].map((item, i) => (
